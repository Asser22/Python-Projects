...
wind.title("Ping Pong By Asser") #set the title of the window
...
score.hideturtle()
score.goto(0, 260)
score.write("Player 1: 0 Player 2: 0", align="center", font=("center",24,"normal"))
...
def madrab1_up():
    ...
    madrab1.sety(y) #set the y of the madrab1 to the nw y coordinate


def madrab1_down():
    y = madrab1.ycor()
    y -= 20 #set y to decrease be 20
    madrab1.sety(y)
...
while True:
    ...
    #move the ball
    ball.setx(ball.xcor() + ball.dx) #ball starts at 0 and everytime loop run-->+0.25 xaxis
    ball.sety(ball.ycor() + ball.dy) #ball starts at 0 and everytime loop run-->+0.25 yaxis

    #border check, top border +300px, botto, border -300px, ball is 20px
    ...
    if ball.xcor() > 390: #if ball is at right border
        ball.goto(0, 0)   #return ball at center
        ball.dx *= -1     #reverse the x direction
        score1 += 1
        score.clear()
        score.write("Player 1: {} Player 2: {}".format(score1, score2), align="center", font=("center",24,"normal"))
